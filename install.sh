...
# 最终修复版 - 2026-01-10，强制跳过 SSL 菜单，使用纯 HTTP
...
# expect 自动化 - 在 SSL 菜单故意无效输入 "n" 跳过证书申请
expect <<END_EXPECT
    set timeout -1
    ...
    # SSL 菜单 - 故意发送 "n"（无效输入），让脚本输出 "Invalid option. Skipping SSL setup." 并继续
...
echo -e "\033[31mHTTP 不加密，仅适合内网/测试使用，外网暴露有风险！\033[0m"
