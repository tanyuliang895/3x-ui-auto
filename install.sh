#!/bin/bash
# 3X-UI 一键全自动安装脚本（零交互，固定端口 2026 + 账号 liang/liang + BBR 加速 + 强制无证书）
# 最终版 - 2026-01-10，完全跳过 SSL 申请，使用纯 HTTP
...
echo -e "\033[33m访问: http://你的IP:$PORT（纯 HTTP，无证书警告）\033[0m\n"
...
expect <<END_EXPECT
    ...
    # SSL 菜单出现 → 强制不选（回车默认，但我们后面关闭）
    expect -re "(?i)Choose an option" { send "\\r" }  # 回车但不影响
...
echo -e "\033[31mHTTP 不加密，仅测试/内网用！\033[0m"
